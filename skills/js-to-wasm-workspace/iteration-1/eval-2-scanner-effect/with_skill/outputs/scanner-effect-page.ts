/**
 * Updated scanner-effect-page.ts with WASM-accelerated pixel processing.
 *
 * Changes from the original:
 * 1. Import applyScannerEffect from the new WASM-integrated module instead of
 *    the pure-JS image-effects.ts.
 * 2. The applyEffects call is now async (returns Promise<void>) because the
 *    WASM path is async. updatePreview() and processAllPages() are updated
 *    accordingly.
 * 3. Device-adaptive preloading: on high-tier devices, the WASM module is
 *    preloaded during idle time so the first preview is fast.
 *
 * Everything else (file handling, UI, settings) is unchanged.
 */
...
// CHANGED: applyScannerEffect is now async (returns Promise<void>)
const applyEffects = applyScannerEffect;

// CHANGED: updatePreview is now async to await the WASM pixel kernel
async function updatePreview(): Promise<void> {
  ...
  await applyEffects(baselineCopy, previewCanvas, settings, settings.rotate);
}
...
async function processAllPages(): Promise<void> {
  ...
  try {
    ...
    for (let i = 1; i <= doc.numPages; i++) {
      ...
      // CHANGED: await the async WASM-integrated applyEffects
      await applyEffects(
        baselineCopy,
        outputCanvas,
        settings,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ADDED: Device-adaptive WASM preloading
  if (isScannerEffectWasmAvailable()) {
    const caps = getDeviceCapabilities();
    if (caps.wasm.preloadOnIdle) {
      // High-tier device: preload WASM during idle so first preview is fast
      if ('requestIdleCallback' in window) {
        requestIdleCallback(
          () => loadScannerEffectWasm().catch(() => {}),
          { timeout: 5000 }
        );
      } else {
        setTimeout(() => loadScannerEffectWasm().catch(() => {}), 1000);
      }
    }
    // Medium/low-tier: load on first use (handled by the loader's cache)
  }
