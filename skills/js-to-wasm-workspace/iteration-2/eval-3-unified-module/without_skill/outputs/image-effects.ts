/**
 * image-effects.ts (updated)
 *
 * Drop-in replacement for the original image-effects.ts. Each public function
 * attempts to use the WASM module for pixel processing and falls back to the
 * original JS implementation if the WASM module is unavailable. This ensures
 * zero regressions: the tool works identically even when WASM fails to load.
 *
 * The WASM module is loaded lazily on the first call that needs it. Callers
 * do not need to change their import paths or call signatures.
 */
...
} from './image-effects-wasm.js';

// ---------------------------------------------------------------------------
// WASM availability helpers
// ---------------------------------------------------------------------------
...
/**
 * Try to load the WASM module. Non-throwing: on failure it silently falls
 * back to JS. Called once, on the first effect invocation.
 */
async function ensureWasm(): Promise<boolean> {
  if (wasmLoadAttempted) return wasmAvailable;
  wasmLoadAttempted = true;
  try {
    wasmInstance = await loadImageEffectsWasm();
    wasmAvailable = true;
  } catch {
    wasmAvailable = false;
  }
...
/**
 * Synchronously check whether WASM is ready (already loaded).
 * Returns the instance if available, null otherwise.
 */
function getWasmSync(): ImageEffectsWasm | null {
  if (wasmAvailable && wasmInstance) return wasmInstance;
  if (isImageEffectsWasmLoaded()) {
    // Module was loaded externally (e.g. via preload)
    wasmAvailable = true;
    wasmLoadAttempted = true;
  }
  return wasmInstance;
...
// ---------------------------------------------------------------------------
// Preload API (optional, for eager loading on high-tier devices)
// ---------------------------------------------------------------------------

/**
 * Eagerly load the WASM module in the background. Can be called during idle
 * time on high-tier devices so that the first applyGreyscale/applyInvertColors
 * call does not pay the load cost.
 */
export async function preloadImageEffectsWasm(): Promise<void> {
  await ensureWasm();
}

// ---------------------------------------------------------------------------
// Pure JS fallback implementations (unchanged from original)
// ---------------------------------------------------------------------------

function jsApplyGreyscale(data: Uint8ClampedArray): void {
...
function jsApplyInvertColors(data: Uint8ClampedArray): void {
  for (let j = 0; j < data.length; j += 4) {
    data[j] = 255 - data[j];
...
/**
 * Convert image to greyscale using luminance weights (BT.601).
 * Uses WASM if loaded, otherwise falls back to JS.
 */
export function applyGreyscale(imageData: ImageData): void {
  const wasm = getWasmSync();
  if (wasm) {
    wasm.apply_greyscale(imageData.data);
  } else {
    jsApplyGreyscale(imageData.data);
  }
}

/**
 * Invert RGB channels (255 - value). Alpha is preserved.
 * Uses WASM if loaded, otherwise falls back to JS.
 */
export function applyInvertColors(imageData: ImageData): void {
  const wasm = getWasmSync();
  if (wasm) {
    wasm.apply_invert_colors(imageData.data);
  } else {
    jsApplyInvertColors(imageData.data);
  }
}

// ---------------------------------------------------------------------------
// Color conversion helpers (kept in JS -- not hot-path per-pixel)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Scanner effect (WASM-accelerated pixel loop + JS canvas operations)
// ---------------------------------------------------------------------------
...
export function applyScannerEffect(
  ...
  // --- Pixel processing: try WASM combined pipeline, fall back to JS ---
  const wasm = getWasmSync();
  if (wasm) {
    wasm.apply_scanner_pixels(
      data,
      settings.grayscale,
      settings.brightness,
      settings.contrast,
  ...
  // --- Border, rotation: canvas API operations (not pixel-level, stay in JS) ---
  if (settings.border) {
...
// ---------------------------------------------------------------------------
// Color adjustments (WASM-accelerated pixel loop + JS canvas operations)
// ---------------------------------------------------------------------------
